"""
Visualize RoamingRooms episodes with model prediction.
...
from sim.visualization.VideoGenerator_Model import (VideoGenerator,
                                                    GeneratorArgs)

log = get_logger()

_ZOOM_ON = False
_VID_OUT_DIR = 'model'  # zoom_on' if _ZOOM_ON else 'zoom_off'
_FORCE_OVERWRITE = True

# import glob
# parent = os.getcwd()
# print(parent)
# [print(ele) for ele in glob.glob(f'{parent}/data/*')]
# exit()
...
def evaluate(model, dataiter, num_steps, verbose=False):
  ...
  for i, batch in zip(it, dataiter):
    vid_path = os.path.join(video_write_dir,
                            'sample_walk_{:03d}.mp4'.format(i))

    if os.path.exists(vid_path) and not _FORCE_OVERWRITE:
      continue
    x = batch['x_s']
    y = batch['y_s']
    y_gt = batch['y_gt']
    ...
    # DEBUG
    # *************************************************************************
    # m = (f'{"class":11s} {"inst_id":12s} {"y_s":15s} {"y_gt":15s}' +
    #     f'{"y_pred":15s}')
    # print(m)
    # print("-"*80)
    stack = [
