  ...
  def __init__(self,
    ...
    with variable_scope(name):
      self._conv1 = Conv2D(
          ...
          dtype=dtype)
      self._gn1 = GroupNorm(
          "gn1", out_filter, num_groups, data_format=data_format, dtype=dtype)
      self._conv2 = Conv2D(
          "conv2",
          3,
          out_filter,
          ...
          dtype=dtype)
      self._gn2 = GroupNorm(
          "gn2", out_filter, num_groups, data_format=data_format, dtype=dtype)
    self._data_format = data_format
    self._add_relu = True
...
class BottleneckResidualModule(ContainerModule):
  ...
  def __init__(self,
    ...
    with variable_scope(name):
      ...
      self._gn1 = GroupNorm("gn1", out_filter // 4, num_groups, dtype=dtype)
      self._conv2 = Conv2D(
          "conv2",
          3,
          out_filter // 4,
          ...
          self._stride_arr(1),
          dtype=dtype)
      self._gn2 = GroupNorm("gn2", out_filter // 4, num_groups, dtype=dtype)
      self._conv3 = Conv2D(
          "conv3",
          1,
          out_filter // 4,
      ...
      if self._ds:
        ...
        self._projgn = GroupNorm("projgn", out_filter, num_groups, dtype=dtype)
  ...
  def forward(self, x, is_training, **kwargs):
    ...
    x = self._gn3(x)
    if self._ds:
      x += self._projgn(self._projconv(origx))
    if self._add_relu:
      x = tf.nn.relu(x)
    return x
  ...
  def __init__(self,
    ...
    with variable_scope(name):
      self._conv = Conv2D(
          ...
          dtype=dtype)
      self._gn = GroupNorm(
          "gn", out_filter, num_groups, data_format=data_format, dtype=dtype)
    self._data_format = data_format
...
@RegisterModule('resnet_gn_backbone')
class ResnetGNBackbone(Backbone):

  def __init__(self, config, dtype=tf.float32):
    super(ResnetGNBackbone, self).__init__(config, dtype=dtype)
    ...
    self._init_conv = InitConvModule(
        ...
        config.num_channels,
        filters[0],
        num_groups=config.num_norm_groups,
        max_pool=config.init_max_pool,
        data_format=config.data_format,
        dtype=dtype)
    self._blocks = [self._init_conv]
