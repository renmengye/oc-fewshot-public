...
def downsizeWithMerge(scan):
    ...
    for pano in pano_ids:
        ...
        for skybox_ix in range(6):
            ...
            skybox_path = skybox_template % (base_dir, scan, pano, skybox_ix)
            #skybox = cv2.imread(skybox_template % (base_dir,scan,pano,skybox_ix))
            skybox = cv2.imread(skybox_path)
        ...
        writepath = skybox_merge_template  % (base_dir, scan, pano)
        assert cv2.imwrite(skybox_merge_template % (base_dir,scan,pano), newimg), f'Fail to write to {writepath}'
...
if __name__ == '__main__':
    with open('data/v1/connectivity/scans.txt') as f:
