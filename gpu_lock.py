...
def _launch_reaper(id, pid):
  ...
  Popen([reaper_cmd, str(pid), me, '--free',
         str(id)],
...
def obtain_lock_id_specify(id, pid=None):
  try:
    if _obtain_lock(id):
      # print(id)
      # print('locked', id)
...
def nvidia_gpu_stats():
  ...
  try:
    ...
    for gpu in gpulist:
      ...
      memlist.append('%03.f' % memused_float + '+%03.f' % memfree_float +
                     '=%03.f' % memtot_float + 'Mb')
...
if __name__ == "__main__":
  ...
  if '--id' in sys.argv:
    ...
    try:
      ...
      sys.exit(1)
    pid = os.getppid()
    print(obtain_lock_id_specify(id, pid))
    # try:
    #     id = int(sys.argv[2])
    # except:
    #     print 'Usage: %s --ids [specific gpu id]' % me
    ...
    print(div)
    for id in board_ids():
      print("      Board %d {Use:%s; Mem:%s; Temp:%s}: %s" %
            (id, stats[0][id], stats[1][id], stats[2][id], owner_of_lock(id)))
    print(div + '\n')
  else:
    stats = nvidia_gpu_stats()
    print(div)
    ...
    print('  More info: http://www.cs.toronto.edu/~nitish/gpu_lock.html')
    print('  Report any problems to: guppy-tsar@cs.toronto.edu')
    print('\n' + div)
    print("  NVIDIA board users:")
    print(div)
    for id in board_ids():
      print(
          "  Board %d {Use:%s; Mem(used+free=total): %s; Fan:%s; Temp:%s}: %s"
          % (id, stats[0][id], stats[1][id], stats[2][id], stats[3][id],
             owner_of_lock(id)))
    print(div + '\n')
